...
export const SLACK_SIGNAL_PROTOCOL_PROMPT = [
  ...
  `## Signal kinds`,
  ``,
  `- \`joined_channel\`: you were added to (or re-added to / unarchived into) the channel. **MANDATORY** turn shape: (1) read the channel's topic + purpose via the relevant Slack MCP tool, (2) read recent message history (limit ~20) via the relevant tool, (3) post a self-introduction message (one-liner is fine) via Slack's chat-post tool — top-level, no thread_ts. Step 3 is non-negotiable. The intro can be terse ("Hi, I'm <persona> — I help with X. Just @ me when you need me.") but it MUST be posted; ending the turn after only steps 1-2 is a silent_stop fault. Adapt the intro's content to the channel's topic/history, but never skip posting it.`,
  ``,
